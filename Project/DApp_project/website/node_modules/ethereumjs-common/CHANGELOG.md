...
- Introduces **support for private chains** by allowing to pass a custom dictionary as the `chain` parameter
  ...
  `chain` `String` names (e.g. `mainnet`, `goerli`), PR [#24](https://github.com/ethereumjs/ethereumjs-common/pull/24)
...
- Updated `goerli` bootstrap nodes, PR [#20](https://github.com/ethereumjs/ethereumjs-common/pull/20)
